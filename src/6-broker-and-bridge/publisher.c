...
/*!
 * \file publisher.c
 * \brief Publisher code to test "topic-more-secure-options" security mechanism
...
#define TOPIC "/topic/broker/and/bridge"
...
int main(int argc, char **argv)
{
    ...
    /**
     * The port number can also be set in command line to choose whether
     * to connect to the broker or the bridge
     */
    const int32_t port = (argc == 2 ? atoi(argv[1]) : BROKER_PORT);
    Logd("Connect client to the broker with port: %d", port);
    mosquitto_connect(mosq, BROKER_HOSTNAME, port, 10);

    Logd("Publish message to topic: %s", TOPIC);
    char payload[] = "Testing \"topic-broker-and-bridge\" security mechanism";
